...
function DEATH_AI(id, playerid)
	...
	if mob then
		if playerid ~= -1 then
			...
			mob.a = respawn
			SetTimerEx("SPAWN_AI", 10000, 0, id)
			mob.focus = false;
			mob.warn = false;
			mob.run = false;
			mob.attack = false;
...
function sleep(id)
  local t = os.clock()
  local resp = Monsters_Stats[mob.special_instance].respawn - 2
  while os.clock() - t <= resp do
    -- nothing
  end
end
...
function OnAnimalAI(bot, id)
	if bot.death then
		...
		elseif bot.focus then
			...
			--local is_inv = IsInvise(bot.focus);
			if IsPlayerConnected(bot.focus) == 1 and foucus_dead == 0 and IsDead(id) == 0 --and is_inv == false
