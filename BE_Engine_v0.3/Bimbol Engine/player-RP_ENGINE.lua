...
function restart_Player(playerid, disconnect)
	...
	Player[playerid].class = "Без класса";
	Player[playerid].derevo = 0;
	Player[playerid].golod = 0;
	Player[playerid].zazda = 0;
	Player[playerid].nugget = 0;
...
function SetPlayerGuild(playerid, guild)
	_SetPlayerGuild(playerid, guild);
	Player[playerid].Guild = guild;
end
...
function givePlayerAllItems(playerid)
	if ENABLE_SAVE_PLAYER_AFTER_DEATH then
		...
		_SetPlayerGuild(playerid, Player[playerid].Guild);
